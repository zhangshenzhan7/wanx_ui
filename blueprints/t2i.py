...
def create_t2i_task():
    ...
    try:
        ...
        def update_t2i_cache(task_id: str, update_data: dict):
            """更新文生图任务缓存"""
            try:
                # 如果成功，下载图片到本地
                if update_data.get('task_status') == 'SUCCEEDED' and update_data.get('image_urls'):
                    local_filenames = cache_service.download_t2i_images(task_id, update_data['image_urls'])
                    if local_filenames:
                ...
                cache_service.update_t2i_task(task_id, update_data)
        ...
        for i in range(batch_count):
            # 对于wan2.6-t2i传递回调函数
            callback = update_t2i_cache if params.get('model') == 'wan2.6-t2i' else None
